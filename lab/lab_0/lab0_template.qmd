...
- Problematic names: `Engine size`, `Fuel type`, and `Year of manufacture` may cause problems because they contain spaces (cloaking in ` means they're strings?).
...
**Your answer:** Tibble prints a nicely formatted table, whereas the traditional dataframe doesn't have column spacing which can make rows/columns difficult to read.
...
**Your answer:** Because Year of manufacture contains spaces.
...
car_data <- car_data %>%
  mutate(
    price_category = case_when(
      Price < 15000 ~ "budget",
      15000 < Price & Price < 30000 ~ "mid-range",
      30000 < Price ~ "luxury"
    )
  )
...
car_data %>% filter(`Fuel type` == 'Diesel' & age < 10, na.rm = TRUE)
