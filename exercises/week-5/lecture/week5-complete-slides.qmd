...
#Challenge: Predict median home value (B25077_001)

m1 <- lm(home_valueE ~ median_incomeE, data = challenge_data)
summary(m1)
#MR2: 0.8685, AR2: 0.8665
#Int ***, Inc ***

m2 <- lm(home_valueE ~ median_rentE, data = challenge_data)
summary(m2)
#Int none, Rent ***
#MR2: 0.8276, AR2: 0.825
#Lower R^2 than med inc

m3 <- lm(home_valueE ~ poverty_rateE, data = challenge_data)
summary(m3)
#Int ***, Pov .
#MR2 0.05048, AR2 0.03589
#bad

m1.1 <- lm(home_valueE ~ median_incomeE + poverty_rateE, data = challenge_data)
summary(m1.1)
# Int ***, Inc ***, Pov **
#MR2 0.8825, AR2 0.8788 (improved from m1)

m1.2 <- lm(home_valueE ~ median_incomeE + poverty_rateE + percent_collegeE, data = challenge_data)
summary(m1.2)
#Int ***, Inc ***, Pov *, College none
#MR2 0.8854, AR2 0.88 (improved over 1.1)

m1.3 <- lm(home_valueE ~ median_incomeE + poverty_rateE + percent_collegeE + total_popE, data = challenge_data)
summary(m1.3)
#Int ***, Inc ***, Pov *, College none, Pop none
#MR2 0.8882, AR2 0.881 (improved over 1.2)

m1.4 <- lm(home_valueE ~ median_incomeE + poverty_rateE + percent_collegeE + total_popE + median_ageE, data = challenge_data)
summary(m1.4)
#Int none, Inc ***, Pov *, College none, Pop ., Age *
#MR2 0.8991, AR2 0.8908 (improved over 1.3)

m1.5 <- lm(home_valueE ~ median_incomeE + poverty_rateE + percent_collegeE + total_popE + median_ageE + median_rentE, data = challenge_data)
summary(m1.5)
#Int none, Inc ***, Pov *, College ., Pop *, Age ., Rent ***
#MR2 0.9184, AR2 0.9103

library(car)
vif(m1.5)
#  median_incomeE    poverty_rateE percent_collegeE       total_popE      median_ageE     median_rentE 
#        7.368702        15.361217        45.828663        88.475077         1.597782         6.981765 
#Suggests removal of pov, college, pop due to multicolliniarity

m1.6 <- lm(home_valueE ~ median_incomeE + median_ageE + median_rentE, data = challenge_data)
summary(m1.6)
# Int none, Inc ***, Age none, Rent ***
#MR2 0.9102, AR2 0.9059 (better than 1.4, worse than 1.5, but far less variables than both)
vif(m1.6)
#median_incomeE    median_ageE   median_rentE 
#      4.530117       1.443014       5.424355 

```
  
```{r}
#PREDICTION approach, rather than inferential.
library(caret)
...
m3.1 <- train(home_valueE ~ median_incomeE + poverty_rateE,
              data = challenge_data,
              method = "lm",
              trControl = train_control)
m3.1$results
...
m3.2 <- train(home_valueE ~ median_incomeE + poverty_rateE + total_popE,
              data = challenge_data,
              method = "lm",
              trControl = train_control)
m3.2$results
#Worse than m3 and m3.1
```
```{r}

m3.3 <- train(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE,
              data = challenge_data,
              method = "lm",
              trControl = train_control)
m3.3$results
#Worse than m3 and m3.1 and m3.2
```
```{r}

m3.4 <- train(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE + median_rentE,
              data = challenge_data,
              method = "lm",
              trControl = train_control)
m3.4$results
#Better than m3 to m3.3
```
```{r}

m3.5 <- train(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE + median_rentE + percent_collegeE,
              data = challenge_data,
              method = "lm",
              trControl = train_control)
m3.5$results
...
challenge_data$residuals <- residuals(m3.5)
challenge_data$fitted <- fitted(m3.5)
...
lm3.5 <- lm(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE + median_rentE + percent_collegeE, data = challenge_data)
...
challenge_data <- challenge_data %>%
  mutate(
    cooks_d = cooks.distance(lm3.5),
    leverage = hatvalues(lm3.5),
...
```{r}
library(lmtest)
nophl_data <- filter(challenge_data, GEOID != 42101)

nophl_lm3.5 <- lm(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE + median_rentE + percent_collegeE, data = nophl_data)

bptest(nophl_lm3.5)
```
```{r}
nophl_m3.5 <- train(home_valueE ~ median_incomeE + poverty_rateE + total_popE + median_ageE + median_rentE + percent_collegeE,
              data = nophl_data,
              method = "lm",
              trControl = train_control)
nophl_m3.5$results
