...
age_vars <- acs_vars_2022 %>% 
  filter(str_detect(label, "[Mm]edian [Aa]ge"))

head(age_vars)

age_concept <- acs_vars_2022 %>% 
  filter(str_detect(concept, "[Mm]edian [Aa]ge"))
...
pfilterprint("Income variables (not family income):")
...
county_data_poplog <- county_data %>% 
  mutate(
    logpop = log(total_popE)
  )
correlationlogpop <- cor(county_data_poplog$logpop, county_data_poplog$median_incomeE, use = "complete.obs")
